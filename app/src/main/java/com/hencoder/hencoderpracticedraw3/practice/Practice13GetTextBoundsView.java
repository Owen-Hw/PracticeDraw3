...
public class Practice13GetTextBoundsView extends View {
    ...
    String[] text = new String[]{"A", "a", "J", "j", "Â", "â"};
    int[] bounds = new int[text.length];
    int top = 200;
    int bottom = 400;
...

    {
        ...
        for (int i = 0, len = text.length; i < len; i++) {
            paint2.getTextBounds(text[i], 0, text[i].length(), boundRect);
            bounds[i] = -(boundRect.top + boundRect.bottom) / 2;
        }
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0, len = text.length; i < len; i++) {
            canvas.drawText(text[i], 100*(i+1), middle+bounds[i], paint2);
        }
    }
}